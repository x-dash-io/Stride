...
export function SubscriptionClient({ initialStatus, initialInvoices, userId }: SubscriptionClientProps) {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto animate-fade-in">
      <div>
        <h1 className="text-4xl font-bold font-serif text-foreground tracking-tight">Store Subscription</h1>
        <p className="text-sm text-muted-foreground mt-1 font-medium">Submit payment reference codes and track your store billing status.</p>
      </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-destructive/20 bg-destructive/10 p-5 backdrop-blur-sm">
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-base font-bold text-destructive tracking-tight">Access Temporarily Suspended</h3>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {/* Status Card */}
        <Card className="border border-border/50 bg-card shadow-sm md:col-span-1">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between text-sm border-t border-border/50 pt-3">
              ...
              <span className="font-semibold text-foreground">{formatPrice(3500)}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Period Start</span>
              <span className="text-foreground font-medium">{format(new Date(billingStatus.currentPeriodStart), 'MMM d, yyyy')}</span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Period End</span>
              <span className="text-foreground font-medium">{format(new Date(billingStatus.currentPeriodEnd), 'MMM d, yyyy')}</span>
            </div>
            <div className="flex items-center justify-between text-sm border-t border-border/50 pt-3">
        ...
        <Card className="border border-border/50 bg-card shadow-sm md:col-span-2">
          ...
          <CardContent className="space-y-6">
            <div className="bg-accent/40 p-5 rounded-xl border border-border/50 space-y-3">
              <h3 className="font-semibold text-sm text-foreground">M-Pesa Payment Instructions:</h3>
              <ol className="list-decimal pl-5 text-sm space-y-2 text-muted-foreground">
                ...
                <li>Enter Till Number: <strong className="text-foreground font-bold">5123456</strong>.</li>
                <li>Enter Amount: <strong className="text-foreground font-bold">KES 3,500</strong>.</li>
            ...
            {billingStatus.status !== 'PAID' && billingStatus.status !== 'WAIVED' ? (
              <form onSubmit={handleSubmitReference} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    className="font-mono text-base uppercase bg-background border-border"
                    maxLength={10}
                    required
                  />
              ...
              <div className="flex items-center gap-3 text-green-700 dark:text-green-400 bg-green-500/10 p-4 rounded-xl border border-green-500/20">
      ...
      <div className="bg-card border border-border/50 rounded-xl overflow-hidden shadow-sm">
        <div className="p-6 border-b border-border bg-gradient-to-r from-accent/50 to-card">
          <h2 className="font-serif font-bold text-lg text-foreground tracking-tight">Invoice & Billing History</h2>
        </div>
